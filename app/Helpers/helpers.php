...
function menu_access($menu_name=''){

    $user_id=\Illuminate\Support\Facades\Auth::user()->id;

    if (\Illuminate\Support\Facades\Auth::user()->user_name!='admin'){
        $menus_permission=DisableMenus::where('menu',$menu_name)->first();
        $emp_ids='';
        if(isset($menus_permission->emp_ids)){
            $emp_ids=$menus_permission->emp_ids;
        }
        ...
        if ($emp_ids==''){
            ...
            $exist=in_array_r($user_id,$emp_ids);
            if ($exist && $menus_permission->status=='active'){
                return true;
            }else{
                return false;
            }
...
function latest_five_employee_leave_application($employee_id=''){
    return Leave::where('status','pending')->where('employee_id',$employee_id)->orderBy('created_at','desc')->take(5)->get();
}

...
class PayrollController extends Controller
{
    ...
    public function payPayment($emp_id,$date)
    {
        ...
        if($payment_type=='Hourly'){
            ...
            $daily_salary=floor(($net_salary/30));
            $a = 0;
            foreach($payment as $p){
                ...
                if($p->status == 'Absent') $a++;
            }
            if($lp>$max_late_present){
                $d = floor($lp/($max_late_present+1));
                $attendance_panelty = $daily_salary*$d;
            }
            $attendance_panelty+=$daily_salary*$a;
            $overtime_salary=round($overtime_salary,2);
        ...
        $payment_amount=$payment_amount-$provident_deducted-$loan_deducted-$deducted_tax-$attendance_panelty;
        $payment_amount=round($payment_amount,2);



        return view('admin.pay-payment',compact('text_date','net_salary','overtime_salary','payment_amount','emp_id','date','payroll','provident_deducted','loan_deducted','deducted_tax','attendance_panelty', 'employee'));
    }
    ...
    public function payPaymentPost(Request $request)
    {
        ...
        $date=date('Y-m-d',strtotime($date));
        $v=\Validator::make($request->all(),[
            'net_salary'=>'required','overtime_salary'=>'required','payment_amount'=>'required','payment_type'=>'required','attendance_panelty'=>'required'
        ]);
        ...
        // echo('<pre>');
        // print_r($request->attendance_panelty);

        $designation=$employee->designation;
        $department=$employee->department;
        $provident_fund=$request->provident_fund;
        $attendance_panelty = $request->attendance_panelty;
        ...
        $payroll=Payroll::firstOrCreate([
            ...
            'loan'=>$loan,
            'attendance_panelty'=>$attendance_panelty,
    ...
    public function downloadPdf($id)
    {
        ...
        $pdf =PDF::loadView('admin.pdf-payslip', $payslip);
        // return View('admin.pdf-payslip',compact('payslip'));

...
class EmployeeController extends Controller
{
    ...
    public function viewEmployee($id)
    {
        ...
        if ($employee) {
            ...
            $device = Device::all();
            return view('admin.view-employee', compact('employee', 'designation','department','bank_accounts','employee_doc','tax','role', 'device'));
        } else {
            return redirect('employees/all')->with([
    ...
    public function postEmployeePersonalInfo(Request $request)
    {
        ...
        if($request->uid){
            Employeetodevice::updateOrInsert(
                ['employee_id' => $employee->id, 'device_id' => $request->device],
                ['uid' => $request->uid]
            );
        }
    ...
    public function getUid(Request $request)
    {
        ...
        if ($users) {
            foreach ($users as $d) {
                echo '<option value="' . $d['userid'] . '">' . $d['name'] . '</option>';
            }
        }
    }
    ...
    /* Show employee connected to device */
    public function employeetodevice()
    {
        ...
        $employeeIndevice = Employeetodevice::all();
        $employee = Employee::where('status', 'active')->where('user_name','!=','admin')->get();
        $department = Department::all();
        $device = Device::all();
        return view('admin.employee-to-device', compact('employeeIndevice', 'employee', 'department', 'device'));
    }
    ...
    public function addEmployeeToDevice(Request $request){
        ...
        $status = Employeetodevice::updateOrInsert(
            ['employee_id' => $request->employee, 'device_id' => $request->device],
            ['uid' => $request->uid]
        );

        if($status){
            return redirect()->back()->with([
                'message'=> language_data('Employee connected to device successfully')
            ]);
        }
        return redirect()->back()->with([
                'message'=> language_data('Faield to insert data!'),
                'message_important'=>true
            ]);
        
    }
    public function removeEmployeeFromDevice(Request $request){
        $validated = $request->validate([
            'device_id' => 'required',
        ...
        // echo("<pre>");
        // print_r($request->all());
        $status = Employeetodevice::where('employee_id', $request->employee_id)->where('device_id', $request->device_id)->delete();

        if($status){
            return redirect()->back()->with([
                'message'=> language_data('Employee remove from device successfully')
            ]);
        }
        return redirect()->back()->with([
                'message'=> language_data('Faield to remove data!'),
                'message_important'=>true
            ]);
